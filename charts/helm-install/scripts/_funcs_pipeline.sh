...
#   None
#######################################
function run_task_helm() {
  local _task_index=${1}
  local _task_content=${2}
  common::debug "running recipe task index #${_task_index}"
  common::debug "running recipe task content:"
  common::debug "${_task_content}"
  if ! helm-install::process_chart "${_task_content}"; then
...
function process_recipe() {
  ...
  local _funcs_business_logic="./_funcs_helm.sh"
  ...
  # process .preTasks
  if ! run_task_array "${_recipe_file}" ".preTasks" "run_task_generic"; then
    common::err "run_task_array .preTasks error"
    return 1
  fi

  # process .helmCharts
  if ! run_task_array "${_recipe_file}" ".helmCharts" "run_task_helm"; then
    common::err "run_task_array .helmCharts error"
    return 1
  fi

  # process .postTasks
  if ! run_task_array "${_recipe_file}" ".postTasks" "run_task_generic"; then
    common::err "run_task_array .postTasks error"
    return 1
  fi
